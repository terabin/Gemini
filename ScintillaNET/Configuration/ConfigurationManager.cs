...
namespace ScintillaNet.Configuration
{
	...
	public class ConfigurationManager : TopLevelHelper
	{
		...
		public void Configure()
		{
			...
			{
				using (Stream s = GetType().Assembly.GetManifestResourceStream("Gemini.files.default.xml"))
					builtInDefault = new Configuration(s, "default", _useXmlReader);
				if (!string.IsNullOrEmpty(_language))
					using (Stream s = GetType().Assembly.GetManifestResourceStream("Gemini.files." + _language + ".xml"))
						if (s != null)
